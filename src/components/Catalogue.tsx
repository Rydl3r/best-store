...
const Catalogue = (): JSX.Element => {
    ...
    const defaultItems =
        [
            ...
            },
            {
                ...
                image: "https://3-chetverti.ru/image/cache/data/dlya_nego_998x954px-340x230.jpg",
                name: "Hot",
                price: 333
            },
            {
                image: "https://thumb.tildacdn.com/tild3637-6536-4166-b665-353064636136/-/resize/796x/-/format/webp/_.png",
                name: "Zhu Zhu",
                price: 260
            },
            {
                image: "https://pizza.od.ua/upload/iblock/043/043ff81c20912fff40b0eb62d575df31.jpg",
                name: "4 meats",
                price: 235
            }
        ]
    ...
    const addItem = (image: string, name: string, price: number): void => {
        if (items.items.map((item: IItem) => { return (item.name) }).indexOf(name) !== -1) {
            alert("Already added!")
        } else {
            setVisible(true)
            setTimeout(() => {
        ...
        <div>
            {history.location.pathname === "/" ?
                <div>
                        ...
                        {defaultItems.map((item: IItem) => {
                            return (
                                ...
                                <Card className="card mx-3 mt-3">
                                    <CardImg top width="100%" src={item.image} alt="Card image cap" className="card_image" />
                                    <CardBody>
                                        <CardTitle tag="h5">{item.name}</CardTitle>
                                        <CardSubtitle tag="h6" className="mb-2 text-muted">{item.price}$</CardSubtitle>
                                        <Button onClick={() => { addItem(item.image, item.name, item.price) }}>Add to cart</Button>
                                    </CardBody>
                                </Card>
                            )
                        })}
                    ...
                    <Alert color="info" isOpen={visible} className="d-flex justify-content-center mt-3 alert">
                        Added item to cart!
                    </Alert>
                </div>
                ...
                <div>
                    ...
                    <div>
                                ...
                                return (
                                    ...
                                    <Card className="card mx-3 mt-3">
                                        ...
                                        <CardBody>
                                            <CardTitle tag="h5">{item.name}</CardTitle>
                                            <CardSubtitle tag="h6" className="mb-2 text-muted">{item.price}$</CardSubtitle>
                                            <Button onClick={() => { deleteItem(item.image, item.name, item.price) }}>Delete from cart</Button>
                                        </CardBody>
                                    </Card>
                                )
                            })}
                        ...
                        <div>
                            {total === 0 ? "" : <h2 className="total d-flex justify-content-center pt-3">Total: {total}$</h2>}
                        </div>
                    </div>

                    <Alert color="danger" isOpen={visible} className="d-flex justify-content-center mt-3 alert">
